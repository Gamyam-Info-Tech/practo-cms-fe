...
const AuthLayout = () => {
  ...
  useEffect(() => {
    if (location.pathname === ROUTES.LOGIN) setScreen("login");
    if (location.pathname === ROUTES.FORGET_PASSWORD) setScreen("forgot");
  }, [location.pathname]);

  // Memoize the screen setter to prevent unnecessary re-renders
  const memoizedSetScreen = useMemo(() => setScreen, []);
    ...
    <div className="relative w-full min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative w-full max-w-md">
        <div className="relative min-h-[500px]">
          ...
          >
            <Login setScreen={memoizedSetScreen} />
          </div>
          ...
          >
            <ForgotPassword setScreen={memoizedSetScreen} />
          </div>
        </div>
      </div>
    </div>
